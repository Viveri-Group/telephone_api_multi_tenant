...
const submit = () => {
    ...
    form.get(route('web.max-capacity-call-logs.index'), {
        preserveScroll: true
    });
};
...
<template>
    <LayoutBox>
        <SearchContainer heading="Search Max Capacity Call Logs" storage-key="hide_max_capacity_log_search_panel">
            <form @submit.prevent="submit">
                <div class="grid grid-cols-12 gap-x-4 gap-y-1">
                      <Input label="Call ID"
                           name="call_id"
                           tip="The unique call id from the shout switch."
